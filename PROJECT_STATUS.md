...
| 19 | System Design | 🚧 **IN PROGRESS (v4, started 2026-07-30)** — **6 of 36 done**, all pushed & live. Section 1 Fundamentals: `estimation` 30KB ✅, `cap-theorem` 42KB ✅, `consistent-hashing` 43KB ✅, `caching` 51KB ✅, `replication` 53KB ✅, `bloom-merkle` 54KB ✅, `pubsub-queues` 61KB ✅ (that's 7 incl. estimation — index.html still 5KB). **NEXT in nav order: `rest-serverless` (5KB), then `microservices` (6KB), `heartbeat-gossip` (5KB), then `index.html` (How to Scale, 5KB) to close section 1.** Uses the **tradeoff-driven SD format** (below), not the language-track format. |
