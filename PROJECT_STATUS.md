...
| — | Spring Boot | ✅ **DONE** — all 20 lessons at full HTML-track depth (300–360 lines each): Core Container ×5, Web Layer ×5, Data Layer ×3, Cross-Cutting ×3 (AOP, Security, Caching/Async), Production ×4 (Testing, Actuator, Resilience/Microservices, Production capstone). Every lesson: "what you'll master" intro → Parts 1–6 → deep-dive senior `.note` → Common Mistakes → 6 tiered interview Qs → 6 graded exercises w/ solutions → Key Takeaways → pager. Link-audited clean; quiz validates 10/200/0. All pushed & live. |
| 5–19 | all others | ⏳ pending (still at v3 depth) — Node.js is next |
