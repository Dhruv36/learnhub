...
| 17 | ASP.NET Core | 🔄 **IN PROGRESS (2026-08-03)** — **16/23 lessons** at v4, 30–47KB each, all `det=12 ex=6`, 0 validation errors. §1 Foundations 5/5 (index, minimal-apis, routing, model-binding, configuration) · §2 The Pipeline 4/4 (middleware, di-config, filters, validation-errors) · §3 Building APIs 4/4 (controllers-mvc, api-design, versioning, openapi) · §4 Data 3/3 (efcore-data, efcore-advanced, caching) · §5 Security 1/4 (authn-authz done). **REMAINING 7:** authorization, security, signalr (rebuild), caching-background→Background Services (rebuild), testing (new), clean-architecture (rebuild), production-readiness (rebuild). Note two v3 files were re-scoped: `di-config` is now DI-only (config split out) and `validation-errors` is now error-handling-only (validation moved to model-binding). |
...
3. 🔄 **ASP.NET Core** (16/23 done) ← **CURRENT** — 7 lessons remain, listed in the v4 table above
