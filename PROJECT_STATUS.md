...
**Done: 18 full tracks (3,600 quiz questions) — ALL language/framework/infra tracks PLUS the full LeetCode Patterns track complete!** LeetCode used a different per-lesson format than other tracks (see `tutorials/leetcode/two-pointers.html` as the template: Concept/Recognition Cues/Template code/3 Practice Problems Easy-Med-Hard w/ solutions, NOT the v3 skeleton) — its 200-Q quiz bank is pattern-recognition style ("given this problem description, name the pattern"). Remaining: System Design only. Its remaining topics are in `tutorials/system-design/nav.js` (Deep Dives ×6 + Case Studies ×12 + Senior/Staff ×7 = 25 more lessons) — source material at `C:\Users\397dh\OneDrive\Desktop\learn\System Design`. Existing System Design lessons (index, estimation, cap-theorem, consistent-hashing, caching) follow the standard v3 skeleton — use that format, not LeetCode's. No quiz-bank exists yet for System Design (old quiz.html is a single small inline quiz) — needs the standard 10×200 quiz-bank-1.js/2.js + hub, same as every other track.
