...
| | |
|---|---|
| **In progress** | **New track: Apache Kafka (`tutorials/kafka/`) — 11 of 22 lessons written (2026-09-14).** The curriculum is its `nav.js`. Build rule: the newest lesson's pager points at `quiz.html` until the next lesson exists, because validate.py fails on broken links; repoint it when the next one lands. Still to do after lesson 22: the two quiz banks (10 sets × 20, then `node tools/quizshuffle.js tutorials/kafka` and quizcheck), then the home-page card in `index.html` (Infra &amp; DevOps section) — the track stays unlinked until then. Version facts relied on: KRaft-only since 4.0; producer `acks=all` + idempotence default since 3.0; `linger.ms` default 5 ms in 4.0; KIP-848 GA in 4.0. **Before this: the v4 depth pass completed 2026-09-11.** Every track measures at recipe with `tools/depth.py`. aspnet finished 2026-09-11 (24/24); on the way, a claim repeated in seven aspnet lessons (that `UseAuthorization` before `UseRouting` "fails open") was found false and corrected — see rule 4. dotnet (23/23), angular and nodejs finished the same day. |
