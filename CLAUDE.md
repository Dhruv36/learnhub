...
25 tracks under `tutorials/<track>/`. Each track = ~20–46 lessons + a 10×20 quiz bank.

---

## ▶️ Resume here

**Current work: none in flight.** The v4 depth pass finished 2026-09-11 and the Apache Kafka track
(new, written to the v4 recipe from scratch) finished 2026-09-17.

| | |
|---|---|
| **In progress** | **None — the TypeScript track is complete (2026-09-18).** 22 lessons in 6 sections + a 200-question bank (quizcheck `10 200 0`, 0 duplicate stems, answers 65/62/73 after `quizshuffle`), linked from the home page beside JavaScript. Every lesson measures at recipe with `tools/depth.py` (median 6 parts / 6 IQs / 191 iq_words / 6 exercises / 14 mistakes / 15 takeaways / 13 tables); all 22 sit at 33-40KB and are recorded in `REVIEWED_THIN`. **Every compiler claim was verified against a real tsc 7.0.2 in the scratchpad, plus Node 24, esbuild and zod 4.6.5** — the track cites error codes and measured output rather than recollection. Rule 4 changed the content in several places: esbuild DOES compile enums and namespaces (the usual folklore is wrong; the real divergence is `const enum`, where mixing tsc's and esbuild's output gives a runtime `SyntaxError`); branded numbers do NOT guard arithmetic (`Cents + Meters` compiles); `isolatedDeclarations` accepts an inferred function-declaration return type, so it is narrower than "annotate every export". TS7 facts used: strict on by default; `target=ES5`, `moduleResolution=Classic/node10`, `alwaysStrict=false`, `esModuleInterop=false` and `allowSyntheticDefaultImports=false` all removed (TS5108); `suppressExcessPropertyErrors` and friends removed entirely (TS5023); `amd`/`umd`/`system` gone from `module`; `tsc --init` now enables `noUncheckedIndexedAccess` and `exactOptionalPropertyTypes`, which `strict` does not; `--generateTrace` emits one `types_N.json` per parallel checker. **Before this: the Apache Kafka track completed 2026-09-17.** |
| **Measured depth** | `python tools/depth.py react javascript html css` — medians below, react is the finished reference |
| **Next tracks** | **mistakes/takeaways top-up (started 2026-09-11).** A full-site `depth.py` run shows several "complete" tracks below 12–16 mistakes / 14–16 takeaways, IQ depth already fine: database-concepts (10/10), linux (10/13), python and sql (takeaways 10), vector-databases (mistakes 9), docker (mistakes 10), system-design (10/12). Append rows only (rule 1e minus the IQ step). **database-concepts DONE 2026-09-11** (15/15, medians 16/16); rule 4 caught six dated or false claims on the way — pg_upgrade keeps stats from PG18, PgBouncer 1.21+ carries prepared statements, plain `REFRESH MATERIALIZED VIEW` is ACCESS EXCLUSIVE, InnoDB does not escalate locks, the PG14 vacuum failsafe, and checksums can be enabled offline (default on in PG18). **linux DONE 2026-09-11** (20/20, medians 15/16); rule 4 fixed a wrong SigIgn mask, a self-contradicting PID-1 Docker example, a misattributed dmesg line and an inode hunt using `-type d`. Its last 8 lessons were extended without a full re-read; that re-read ran 2026-09-13 (see "Linux re-read" below). **python and sql DONE 2026-09-11** (medians 14/16 and 15/16); their new takeaways mostly promote topics already in each lesson's mistakes table. **vector-databases DONE 2026-09-11** (median 14/16). **docker DONE 2026-09-11** (21/21, median 14 mistakes; takeaways were already 18–20); rule 4 caught runtime-stack claiming containers survive a dockerd restart by default — the default is `live-restore: false`, the shim only makes live-restore possible (lesson, exercise 2, IQs and quiz-bank-1 Q45 corrected) — and process-model saying `on-failure` restarts after a daemon restart (it does not). **system-design DONE 2026-09-11** (36/36, median 13/14; every lesson 12–13 / 14–15). kubernetes, leetcode, mongodb, redis and springboot were already at target (medians 12–13 / 17–20), and a site-wide per-lesson scan left only java pattern-matching and redis index under 12 mistakes, both topped up. **The top-up is COMPLETE** — every lesson on the site now meets 12+ mistakes / 14+ takeaways. `tools/pending/lib.py` now accepts numbered `<h2>8. Common Mistakes</h2>` headings and whitespace inside the takeaways div. **Short-IQ pass DONE 2026-09-11**: the 15 lessons with an iq_words median under 150 (java optional/annotations-reflection/types/modern-java, mongodb query-tuning/explain-plans/aggregation/aggregation-advanced/schema-validation, python control-flow/data-structures/index, springboot security/autoconfig/validation) now sit at 212–242, done by 10 parallel agents with rule 4 checks. The biggest correction was mongodb's sort limit: 100 MB since 4.4 (not 32 MB), and since 6.0 an oversized sort spills to disk by default; about 20 statements were fixed across the track. `shardsTargeted` was presented as an explain field but is not one, so it is now reworded as a count of explain's `shards` entries. The explain-plans LIMIT/SORT trees were fixed 2026-09-13: with a blocking sort the limit folds into SORT as `limitAmount` (a top-k sort bounds memory, not reads); a real LIMIT stage appears only over an index-ordered scan. `validate.py` needs a track path (`python validate.py tutorials/<track>`). No measured gap remains (tables are low only in the elaboration tracks, which are accepted as DONE). **Linux re-read (2026-09-13): COMPLETE, 8 of 8** — text-processing, find-xargs, systemd, logging, namespaces-cgroups, performance, shell-scripting and hardening, with rule-4 fixes throughout. Trailing `# comments` are not comments in systemd units, sysctl.d or logrotate, so 49 such lines across 7 lessons were moved onto their own lines. The other fixes: StartLimit* keys belong in `[Unit]`; `Persistent=` only affects `OnCalendar=` timers; `systemd-cat` does not parse KEY=VALUE, `logger --journald` does; `pidstat -t` puts %CPU in `$9`; there are eight namespaces, not seven; find's `-exec +` batches at 128 KiB, not ARG_MAX. **Both follow-ups done 2026-09-13; no pending work remains.** shell-scripting (verified in bash 5.2): a plain `result=$(false)` DOES exit under `set -e`; only `local`/`export`/`echo "$(…)"` mask it, and bash turns `-e` off inside `$( )` unless `shopt -s inherit_errexit`. An EXIT trap keeps the pre-trap status unless the handler calls `exit N` or trips `set -e`. quiz-bank-2 set 9 had both claims and is corrected too. hardening (sshd verified with OpenSSH 9.7 `sshd -T`): the sshd drop-in must be `00-`, not `99-`, because sshd keeps the first value, so `50-cloud-init.conf` beat it. The other hardening fixes: `ufw limit 22/tcp` beside a `10.0.0.0/8` allow opened SSH to everyone; the "revert" timer only restarted ssh; `df -P`/`-i` are rejected with `--output`; restart does not drop sessions (reload is safer only because Debian's unit re-validates). The AllowGroups comment is on its own line. system-design's "parts 0" is its `<h2>1. …</h2>` numbering, not a gap. |
| **Complete** | typescript, kafka, aspnet, dotnet, angular, nodejs, css, html, javascript, react, java, springboot, python, linux, sql, aws, cicd, docker, kubernetes, mongodb, redis, leetcode, system-design, database-concepts, vector-databases |
